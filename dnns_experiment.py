...
if __name__ == '__main__':
    ...
    estimator = SkTransformedDnnModel(X.shape[1])#X.shape.view(X.shape[1], -1))#torch.from_numpy(np.asarray(X.shape)).view(-1,1))
    ...
    study = optuna.create_study(
        study_name=f'DNN', # f'DNN-CV-{fold}'
        direction='maximize',
    ...
    print(f"CV {0} | {', '.join(['{} = {}'.format(k, v) for k, v in cv_results.items()])}") #fold replaces 0
